...
franchise<-ff_franchises(league)%>%
  separate(user_name,c('user_first','user_last'))
...
roster_waiver[is.na(roster_waiver)] <- 1
...
finalkeeper<-roster%>%
 select(franchise_id,player_name,team,pos,keeper_value)%>%
